...
01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48"""
  ...
  def baselineAnswer: Long = {
    ...
    for {
      ...
      if (c + 3 < cols) {
        val p = grid(r)(c).toLong * grid(r)(c + 1) * grid(r)(c + 2) * grid(r)(c + 3)
        if (p > best) best = p
      }
      if (r + 3 < rows) {
        val p = grid(r)(c).toLong * grid(r + 1)(c) * grid(r + 2)(c) * grid(r + 3)(c)
        if (p > best) best = p
      }
      if (r + 3 < rows && c + 3 < cols) {
        val p = grid(r)(c).toLong * grid(r + 1)(c + 1) * grid(r + 2)(c + 2) * grid(r + 3)(c + 3)
        if (p > best) best = p
      }
      if (r - 3 >= 0 && c + 3 < cols) {
        val p = grid(r)(c).toLong * grid(r - 1)(c + 1) * grid(r - 2)(c + 2) * grid(r - 3)(c + 3)
